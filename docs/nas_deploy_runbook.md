...
| `GHCR_TOKEN` | **패키지가 비공개일 때만 필요.** `read:packages` 스코프 PAT — 현재는 public이므로 **비워 둔다.** 값이 남아 있으면 PAT 만료 시 배포가 깨진다 (§7-3) |
...
### 7-3. 컨테이너 패키지 공개 범위 — **public 유지** (2026-08-13 확정)

GHCR 패키지는 **저장소와 별개의 공개 범위**를 갖는다. 저장소가 공개여도 패키지는 비공개로 둘 수 있다. 검토 결과 **public으로 둔다.** 아래 전환 절차는 재검토 조건에 해당할 때를 위한 것이며, 지금 수행할 작업이 아니다.

**public으로 두는 근거**

- **이미지에 저장소에 없는 것이 없다** — `hub/Dockerfile`은 `pyproject.toml`·`README.md`·`src`만 COPY하고, `.dockerignore`가 `.env`·`deploy`·`docs`·`tests`·`.hyeseongkit/`을 뺀다
- **추가로 드러나는 것은 해결된 의존성 버전뿐**이다(`pyproject.toml`의 범위가 아니라 실제 설치된 값). 그런데 그 정보가 쓸모 있으려면 공격자가 이미 망 안에 있어야 하고, 그 위치라면 무인증 `/healthz`로 버전을 이미 얻는다 — 막아서 얻는 것이 거의 없다
- 반면 비공개는 **PAT 만료라는 새로운 실패 모드**를 만든다 (아래 ⛔)

**재검토 조건** — 하나라도 해당하면 다시 판단한다

1. 저장소를 private으로 전환하는 경우
2. 이미지에 저장소에 없는 것이 들어가는 경우 (빌드 시 주입되는 자격증명, 생성된 설정 파일 등)

> ⛔ **public을 유지하는 동안 `.env`의 `GHCR_TOKEN`은 반드시 비워 둔다 — 지우는 것이 안전하다.**
> `deploy.sh`는 `set -eu` 아래에서 이 값이 비어 있지 않으면 **무조건** `docker login`을 시도한다(30~33행). 공개 패키지라 로그인이 필요 없는데도, **PAT이 만료되면 바로 그 지점에서 배포가 중단된다.** pull은 성공했을 배포가 쓰지도 않는 자격증명 때문에 죽는 것이다.

#### 전환 절차 (재검토 조건에 해당할 때만)

비공개로 두면 **해결된 의존성 버전**이 드러나지 않아, 어떤 CVE가 적용되는지 알아내는 비용이 올라간다.
