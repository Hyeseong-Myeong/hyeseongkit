...
| §11 **D8 볼트 E2E 암호화** | ~~LiveSync 설정 변경이라 이 시스템 밖의 결정~~ → **§0-2-3에서 결론 확정 (2026-08-13).** 설정 작업 자체는 P4 착수 시 | ✅ §0-2-3 |
...
### 0-2-3. ✅ D8 해결됨 — 볼트 E2E 암호화 (2026-08-13 확정)

**결정: 볼트 E2E를 활성화한다. 시점은 P4 착수 시이며, livesync-bridge를 기동하기 전이다.**

D29는 SSOT(`hyeseongkit_sessions`)를 암호화하지만, §8-1 렌더러는 **복호화한 평문 마크다운**을 `/vault-out`에 쓰고 브리지가 그것을 `hyeseongkit_vault`에 넣는다(위 §0-2-2 마지막 구현 요점). 즉 **세션 본문 전체가 파생 뷰에 평문으로 존재한다.** §12-4의 1차 백업은 CouchDB 데이터 폴더를 통째로 뜨므로 볼트 DB도 그 백업에 들어간다 — D29가 명시한 방어 목표(NAS 물리 접근·디스크 탈취·백업 매체 유출)가 파생 뷰를 통해 그대로 뚫린다.

**적용 대상은 네 곳이고 passphrase는 하나다.** CouchDB(서버)는 암호문을 그대로 저장할 뿐이라 **설정 변경이 없다.**

| 대상 | 역할 | 설정 |
|---|---|---|
| livesync-bridge (NAS) | `/vault-out` → 볼트 DB **생산자** | `config.json`에 passphrase — **필드명 미실측 (U2)** |
| Obsidian 데스크톱 | 소비자 | 세션 볼트에서 E2EE + passphrase |
| Obsidian 맥북 | 소비자 | 동일 |
| Obsidian 모바일 | 소비자 | 동일 |

**적용 순서 (브리지 기동이 마지막이어야 한다):**

1. **세션 전용 볼트를 새로 만든다** — 위키 볼트와 별개의 빈 볼트
2. 그 볼트에서 LiveSync를 `hyeseongkit_vault`에 연결, E2EE·passphrase·Property Encryption 설정 → **Configure And Change Remote** (원격이 비어 있어 안전)
3. 맥북·모바일에서 **같은 passphrase** → **Configure**
4. 브리지 `config.json`에 같은 passphrase 반영
5. **그 다음에** 브리지를 기동한다

> ⚠️ **위키 볼트에서 절대 수행하지 않는다.** E2EE 설정은 Obsidian **볼트 단위**이고, `Configure And Change Remote`는 그 볼트의 **원격을 재구축**한다. 위키 볼트에서 실행하면 `<WIKI_VAULT_DB>`가 재구축된다 — D4 (C)가 물리적으로 격리하기로 한 바로 그 DB다.

**구현 요점:**
- **Property Encryption을 켠다.** 렌더 결과 frontmatter에 `title`(한국어 제목)·`sensitivity`·`project`가 들어간다(§8-2). 끄면 이 값들이 평문으로 남는다
- **passphrase는 `HK_ENCRYPTION_KEY`와 같은 급으로 다룬다** — 분실 시 볼트 복구 불가. §12-4 원칙대로 **데이터 백업과 같은 세트에 넣지 않는다**
- **경로는 암호화 대상이 아니다.** E2EE는 문서 내용을 암호화하며, 문서 ID(= 파일 경로 = thread ID)는 path obfuscation이라는 별도 기능이다. 적용하지 않으면 slug가 그대로 보이므로, §0-2-2 마지막 구현 요점과 D30의 "slug에 민감한 표현을 넣지 않는다" 원칙이 **계속 유효**하다
- `/vault-out`의 **파일 자체**는 NAS 디스크에 평문으로 남는다. LiveSync E2EE는 CouchDB 문서를 암호화하는 것이지 소스 파일이 아니다 — 디스크·볼륨 암호화는 별개 판단

**기각: API 계층 E2E (허브가 본문을 못 읽게 하는 방식).** 렌더러(§8-1), packet 조립(§3-6 예산 배분), `hk search`(§3-9 — 허브가 뷰를 복호화해 메모리 매칭) 셋 모두 허브의 복호화 능력에 의존한다. E2E로 가면 허브가 저장소로 격하되고 이 셋을 전부 클라이언트로 옮겨야 한다. 막으려는 위협은 "살아 있는 NAS 침해"인데, 백업 매체 유출은 D29 + 키 분리(§12-4)로 이미 막힌다 — 비용 대비 실익이 맞지 않는다. 전송 구간은 Tailscale(WireGuard)이 이미 암호화한다.
...
- **재시도 백오프 (2026-08-13 신설).** `attempts`는 경과 시간이 아니라 **flush 호출 횟수**를 센다. 간격이 없으면 NAS가 몇 분 재부팅하는 동안 명령을 몇 번 쓰는 것만으로 시도가 소진돼, **짧은 장애가 영구 실패로 기록된다**. 항목에 `next_attempt`를 두어 그 시각 전에는 건너뛴다 — 대기 분 `1 → 5 → 30 → 120 → 360 → 1440`(상한). `MAX_ATTEMPTS = 8`이라 소진까지 실제 경과가 이틀을 넘는다
- `hk queue --flush`는 사용자가 직접 지시한 재전송이므로 **백오프를 무시한다**(`force=True`)
- 시도를 다 쓴 파일은 `failed_at`을 기록하고 `queue/failed/`로 이동한다 — **최종 실패**이며 자동 재시도 대상에서 빠진다. `hk resume`·`hk status`가 건수를 표시한다
- `next_attempt`가 없는 구 형식 파일은 즉시 재시도 대상으로 본다
...
> ⚠️ **키를 데이터 백업과 같은 세트에 넣지 않는다 (2026-08-13 사용자 확정).**
> D29가 막으려는 위협 중 하나가 **백업 매체 유출**인데, 한 세트 안에 암호문과 키가
> 함께 있으면 방어가 성립하지 않는다 — 유출자가 둘 다 갖는다. 위 표의 "별개 경로"는
> 보관 위치만 말하므로, 백업 구성에서 다음을 따로 지킨다.
>
> - **1차(물리) Hyper Backup 대상에 `<DEPLOY_DIR>/.env`를 포함하지 않는다.** `.env`에
>   `HK_ENCRYPTION_KEY`가 있고, 같은 대상에 CouchDB 데이터 폴더가 들어간다
> - `.env`·`jenkins_home`을 백업할 때는 **CouchDB 데이터와 다른 대상·다른 매체**로 분리한다.
>   위 표의 "Hyper Backup 대상에 추가"가 이 분리를 어기지 않는지 확인한다
> - 분리가 실제로 지켜졌는지는 **복원 검증에서 키 없이 복원해 본문이 읽히지 않는 것**으로
>   확인한다 — 읽힌다면 키가 어딘가 백업 안에 함께 들어갔다는 뜻이다
